...
3.  **オンライン周波数推定**: 位相勾配情報を用いて外乱の真の周波数をリアルタイム学習・追従
4.  **位相補正**: モデル位相と観測位相のズレ（位相誤差）を即時補正
5.  **予測制御**: 推定モデルを用いて数ミリ秒先（遅延補償分）の外力を予測
6.  **姿勢補正**: 予測外力を打ち消すような姿勢角（ロール・ピッチ）補正量を生成
...
*   **吊り荷の揺れ学習時**: スイッチ **ON** (High, PWM > 1700)
    *   周波数のオンライン推定が作動。10~30秒程度維持して周波数を学習させる。
*   **学習完了後**: スイッチ **OFF**
