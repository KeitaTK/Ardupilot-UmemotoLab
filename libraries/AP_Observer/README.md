...
### 6.2 共有周波数還元の詳細ロジック（実装ベース）

本実装での共有周波数還元は、以下の流れで行われる。算出された共有周波数 $\bar\omega$ は、信頼度の低い軸に対して選択的に注入される。

#### 6.2.1 融合重みの再定義

各軸の「信頼度」は、以下の4要素の積で定義される（`compute_axis_fusion_weight()` に対応）。
