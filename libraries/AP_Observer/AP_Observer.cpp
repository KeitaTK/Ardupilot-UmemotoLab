...
void AP_Observer::init(){
    ...
    _payload_filter.set_cutoff_frequency(sample_freq, FILTER_CUTOFF_FREQ);
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "AP_Observer: initialized with %.1fHz filter", FILTER_CUTOFF_FREQ);
}
