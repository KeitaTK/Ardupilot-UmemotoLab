...
static void mavlink_test_autopilot_version(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_autopilot_version_t packet1, packet2;
        ...
        packet1.uid_taki = packet_in.uid_taki;
...
static void mavlink_test_taki_custome1(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_TAKI_CUSTOME1 >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_taki_custome1_t packet_in = {
        963497464,17443,17547
    };
    mavlink_taki_custome1_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.test_counter = packet_in.test_counter;
        packet1.taki_system_id = packet_in.taki_system_id;
        packet1.taki_component_id = packet_in.taki_component_id;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_TAKI_CUSTOME1_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_TAKI_CUSTOME1_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_taki_custome1_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_taki_custome1_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_taki_custome1_pack(system_id, component_id, &msg , packet1.test_counter , packet1.taki_system_id , packet1.taki_component_id );
    mavlink_msg_taki_custome1_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_taki_custome1_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.test_counter , packet1.taki_system_id , packet1.taki_component_id );
    mavlink_msg_taki_custome1_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_taki_custome1_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_taki_custome1_send(MAVLINK_COMM_1 , packet1.test_counter , packet1.taki_system_id , packet1.taki_component_id );
    mavlink_msg_taki_custome1_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("TAKI_CUSTOME1") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_TAKI_CUSTOME1) != NULL);
#endif
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_taki_custome1(system_id, component_id, last_msg);
