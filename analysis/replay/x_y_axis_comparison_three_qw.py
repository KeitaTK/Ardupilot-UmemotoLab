#!/usr/bin/env python3
"""Compare X-only, Y-only, and combined (XY) EKF estimates for representative q_w values.

Produces per-window, per-q_w figures (top: PL signals + SW, bottom: EstFreq traces for X/Y/XY)
and a metrics CSV summarizing mean/std/mae/p95_step for each axis/mode.
"""
...
def make_q_plot(tag: str, csv_input: Path, traces: Dict[str, np.ndarray], out_png: Path) -> None:
    ...
    for label, arr in traces.items():
        color = "tab:orange" if label == "x" else ("tab:green" if label == "y" else "tab:blue")
        ax_btm.plot(t, arr, linewidth=1.0, color=color, label=f"{label.upper()} estimate")

    ax_btm.axhline(TARGET_HZ, color="black", linestyle="--", linewidth=1.0, label=f"Target={TARGET_HZ:.2f}Hz")
    ax_btm.set_title(f"{tag}: X/Y/XY estimates")
...
def main() -> int:
    ...
    for win in windows:
        ...
        for q_w in args.qws:
            ...
            # make figure for this q_w
            out_png = fig_dir / f"{win.tag}_q{q_w}_x_y_xy.png"
            make_q_plot(f"{win.tag} q={q_w}", csv_input, traces, out_png)
